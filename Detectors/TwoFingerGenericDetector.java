...
public /*TODO abstract*/class TwoFingerGenericDetector extends AbstractGesture{	
	...
	{
		super(2, GestureDetectorType.trigger_detector, GestureCategory.two_finger_generic );
	}
	...
	public GestureDetectorStatus detect( MotionEvent evnt ){
		if( trigger_finger_count == evnt.getPointerCount() ){
			...
			if( (GestureDetectorStatus.Gesture_listening == mStatus || GestureDetectorStatus.Gesture_dismiss == mStatus ) && meetActiveThreshold(evnt) ){
				mStatus = GestureDetectorStatus.Gesture_active;
				active_not_trig = false;
			}else if( ( GestureDetectorStatus.Gesture_active == mStatus || GestureDetectorStatus.Gesture_moving == mStatus ) && meetMovingThreshold(evnt) ){
				mStatus = GestureDetectorStatus.Gesture_moving;
			}
		}
		else{
			if( mStatus == GestureDetectorStatus.Gesture_active || mStatus == GestureDetectorStatus.Gesture_moving ){
		...
		if( GestureDetectorStatus.Gesture_active == mStatus ){
			return active_not_trig ? mStatus :GestureDetectorStatus.Gesture_idle;
		}
	...
	public Parcelable getGestureInfo(){
		...
		GenericTwoFingerInfoData gInfo = new GenericTwoFingerInfoData(gMessage, 1, 1);
